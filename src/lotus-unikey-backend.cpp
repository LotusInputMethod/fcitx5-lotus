...
private:
    ...
    void syncState(KeySym sym) {
        ...
        if (uic->bufChars() > 0) {
            preeditStr_.append(reinterpret_cast<const char*>(uic->buf()),
                               static_cast<size_t>(uic->bufChars()));
    ...
    bool dispatch(uint32_t sym, uint32_t state) {
        ...
        if (st.testAny(KeyState::Ctrl_Alt) || rawSym == FcitxKey_Control_L || rawSym == FcitxKey_Control_R ||
            rawSym == FcitxKey_Tab || rawSym == FcitxKey_Return || rawSym == FcitxKey_Delete ||
            rawSym == FcitxKey_KP_Enter || (rawSym >= FcitxKey_Home && rawSym <= FcitxKey_Insert) ||
        ...
        if (st.test(KeyState::Super))
            return false;
        if ((rawSym >= FcitxKey_Caps_Lock && rawSym <= FcitxKey_Hyper_R) || rawSym == FcitxKey_Shift_L ||
            rawSym == FcitxKey_Shift_R)
            return false;
        ...
        if (rawSym == FcitxKey_BackSpace) {
            ...
            if (uk_->context()->bufChars() > 0)
                preeditStr_.append(reinterpret_cast<const char*>(uk_->context()->buf()),
                                   static_cast<size_t>(uk_->context()->bufChars()));
            return true;
        }
